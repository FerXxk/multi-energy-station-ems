...
#let index(title: [Índice], target: heading) = context {
  ...
  for item in items {
    ...
    if item.at("level", default: none) != none {
      ...
      // El número de figura/tabla va en la columna de la izquierda; el pie
      // muestra solo su texto, sin repetir "Figura N:" dentro de la entrada.
      show figure.caption: it => it.body

      let figure_idx = numbering("1", ..item.counter.at(loc))
      let figure_number = [#chapter-at(loc).#figure_idx.]

      let entry_f = link(loc)[#grid(
        columns: (4em, 1fr, 1em, 1em),
        figure_number, item.caption, h(1em), page_number,
      )]
