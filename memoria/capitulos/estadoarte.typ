...
Dos tecnologías vehiculares limpias coexisten en este escenario: los vehículos eléctricos de batería (BEV, _Battery Electric Vehicles_) y los vehículos de pila de combustible de hidrógeno (FCEV, _Fuel Cell Electric Vehicles_). Su grado de madurez, sin embargo, no es comparable: el BEV es la tecnología consolidada y de despliegue masivo, mientras que el FCEV sigue en una fase muy anterior y su nicho previsible a corto plazo son los trayectos de larga distancia y los vehículos pesados, donde su mayor autonomía y su repostaje más rápido compensan @iea2023hydrogen.
...
En el ámbito académico, los autores de @roslan2024technoeconomic evalúan en HOMER Pro una estación de recarga de vehículo eléctrico alimentada por fotovoltaica y eólica con baterías de ion-litio, tanque de hidrógeno, electrolizador y pila de combustible, en tres emplazamientos de Malasia, y obtienen un coste neto actualizado (NPC) de entre 1,4 y 3,4 millones de dólares y un coste de la energía (COE) de entre 0,03 y 0,16 \$/kWh según la ubicación. En el plano del control, en @zhu2024pvh2dempc se modela una microrred de corriente continua PV--hidrógeno a nivel de convertidor, con controladores locales para el campo fotovoltaico, el electrolizador y la pila de combustible coordinados mediante control predictivo económico distribuido, y se muestra que este alcanza un comportamiento económico comparable al del control centralizado equivalente con una carga computacional y una oscilación de potencia sensiblemente menores.
...
donde la densidad $ρ$ del hidrógeno a alta presión se obtiene de la ecuación de van der Waals o tablas NIST; a presión atmosférica es de solo 0.09 kg/m³.
...
En el contexto de una estación de repostaje multi-energía, la pila de combustible tiene dos roles complementarios: actuar como generador de respaldo cuando la generación solar es insuficiente y el precio de la red es elevado, y proporcionar energía de arranque cuando la batería está descargada.
...
En @garciatorres2015mpch2 se formula la gestión de una microrred basada en hidrógeno con almacenamiento híbrido como un problema de control predictivo que maximiza el beneficio económico minimizando simultáneamente la degradación de los equipos de almacenamiento, modelando la dinámica continua y discreta mediante _mixed logic dynamics_ y resolviendo el problema resultante como un MIQP. Es un antecedente directo del enfoque de este trabajo en dos sentidos: incorpora explícitamente el desgaste de los equipos como objetivo (y no solo el coste), y procede del mismo grupo de investigación que el modelo de partida. En @apostolou2019hrs se revisa el estado de las estaciones de repostaje de hidrógeno y su infraestructura, y se señala que el coste de producción y compresión del hidrógeno sigue siendo el factor dominante en la viabilidad de estas instalaciones, lo que justifica que la decisión económica «producir localmente o comprar fuera» ocupe un lugar central en el EMS.

En cuanto a la integración conjunta de ambos vectores en una misma instalación, los autores de @tahir2024hybridstation proponen un marco de diseño de estación híbrida que atiende simultáneamente carga de vehículo eléctrico y repostaje de hidrógeno evaluando múltiples atributos de diseño, lo que confirma que el problema de coordinar ambos servicios bajo un único sistema de gestión sigue siendo una línea abierta.
...
Los modelos LSTM y sus variantes (BiLSTM, TCN, Transformer) han desplazado progresivamente a los modelos estadísticos clásicos (ARIMA, SARIMA) y econométricos (GARCH) en las aplicaciones de predicción de precio de electricidad, especialmente a partir de 2018. En @lago2021benchmarking se realiza el benchmark más completo hasta la fecha y se comparan 27 métodos diferentes sobre 8 mercados europeos: los modelos basados en redes neuronales profundas superan a los clásicos en 6 de los 8 mercados, con una reducción media del MAE del 12--28%.
