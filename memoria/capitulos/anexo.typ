...
#let url-repo = "https://github.com/USUARIO/REPOSITORIO"
...
#figure(
  table(
    ...
    [`SimugridElectrolinera/`], [El modelo `OASIS.slx`, su script de preparación `init_OASIS.m` y las dos S-Functions de previsión, `lstm_sol.m` y `lstm_precios.m`.],
    [`codigo/EMS/`], [Las Versiones A, B y C del EMS, más C0. Cada una es el archivo que se pega en el bloque `MATLAB Function` del modelo. La constante `H2_PRECIO_EXT_KG` viene a los 8 €/kg de la campaña que se reporta; a 6,12 €/kg se obtiene la de sensibilidad.],
    [`codigo/herramientas/`], [La campaña de simulación, el cálculo de indicadores, el contraste pareado, la descomposición de la importación y la generación de las figuras del @cap-resultados.],
    [`codigo/preparar/`], [Generación del perfil estocástico de demanda y construcción del _workspace_ de Simulink a partir de los datos.],
    ...
    [`codigo/python/`], [Descarga y análisis de los datos de NASA POWER, ENTSO-E, DESL-EPFL y Caltech, y las figuras del @cap-datos-ev.],
    [`codigo/data/`], [Los datos descargados y procesados que alimentan la simulación.],
    [`codigo/resultados/`], [Los CSV con los indicadores de todas las simulaciones y sus agregados, de los que salen las tablas y las figuras del @cap-resultados.],
